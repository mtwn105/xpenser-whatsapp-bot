  ...
  onPaste(event: ClipboardEvent) {
    ...
    if (clipboardData) {
      ...
      otpArray.forEach((char, index) => {
        const input = document.querySelectorAll('.otp-input')[index] as HTMLInputElement;
        if (input) {
          input.value = char;
        }
      });
      const lastInput = document.querySelectorAll('.otp-input')[otpArray.length - 1] as HTMLInputElement;
      lastInput?.focus();
    }
  }
